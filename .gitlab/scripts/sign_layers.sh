...
#
# LAYER_FILE=dd_trace_dotnet_amd64.zip .gitlab/scripts/sign_layers.sh sandbox
...
if [ ! -f ".layers/tracer.zip" ]; then
    echo "[ERROR]: .layers/tracer.zip not found."
    exit 1
fi
...
echo "---"
echo "Signing layer for tracer.zip"
...
aws s3 cp ".layers/tracer.zip" $S3_UNSIGNED_ZIP_URI
...
aws s3 cp $S3_SIGNED_ZIP_URI ".layers/tracer.zip"
...
echo "---"
echo "Successfully signed tracer.zip!"
